            ...
            if request.method == 'POST':
                ...
                if not form.is_valid():
                    ...
                    for match in ride.ridematches_set.all() :
                        send_mail('Modification d\'un covoiturage',u'Un covoiturage concernant votre trajet entre %s et %s a été modifié et donc supprimé pour éviter des informations erronées.\n Rendez vous sur %slocation/passenger/search/%d/ pour refaire une recherche afin de trouver de nouvelles opportunités de covoiturage' % (match.passenger_ride.start.city_name,match.passenger_ride.dest,SITE_HOST,match.passenger_ride.id),'noreply@bervoets.covoiturage',[u'%s'%match.passenger_ride.passenger.email])
                        match.delete()
                return HttpResponseRedirect('/location/ride/')
            else:
...
def confirm_match(request,match_id):
    try:
        ...
        if request.user == match.driver_ride.driver:
            ...
            send_mail(u'Covoiturage accepté par %s'%match.driver_ride.driver.username, u'%s a accepté la demande de covoiturage suivante : %slocation/ride/matches/%d/ .\n Veuiller prendre contact avec lui pour conclure l\'arrangement.'%(match.driver_ride.driver.username,SITE_HOST,match.id),'noreply@bervoets.covoiturage',[u'%s'%match.passenger_ride.passenger.email])
            return HttpResponseRedirect('/location/ride/')
        else:
...
def deny_match(request,match_id):
    try:
        match = RideMatches.objects.get(pk=match_id)
        send_mail(u'Covoiturage refusé par %s'%match.driver_ride.driver.username, u'%s a refusé la demande de covoiturage suivante : %slocation/passenger/%d/ .'%(match.driver_ride.driver.username,SITE_HOST,match.passenger_ride.id),'noreply@bervoets.covoiturage',[u'%s'%match.passenger_ride.passenger.email])
        if request.user == match.driver_ride.driver:
            if match.accepted:
...
def cancel_match(request,match_id):
    try:
        ...
        if request.user == match.driver_ride.driver or request.user == match.passenger_ride.passenger:
            ...
            match.save()
            send_mail('Suppression d\'un covoiturage',u'Un covoiturage concernant votre trajet entre %s et %s a été supprimé par un des deux partis.\n Rendez vous sur %slocation/ride/search/%d/ pour refaire une recherche afin de trouver de nouvelles opportunités de covoiturage' % (match.passenger_ride.start.city_name,match.passenger_ride.dest,SITE_HOST,match.passenger_ride.id),'noreply@bervoets.covoiturage',[u'%s'%match.passenger_ride.passenger.email])
            send_mail('Suppression d\'un covoiturage',u'Un covoiturage concernant votre trajet entre %s et %s a été supprimé par un des deux partis. Vous n\'avez donc plus à aller prendre %s.\n Rendez vous sur %slocation/ride/edit/%d/ pour mettre à jour vos informations. Vous devez enregistrer les modifications afin que les renseignements concernant les distances soient mis à jour.' % (match.driver_ride.start.city_name,match.driver_ride.dest,match.passenger_ride.passenger.username,SITE_HOST,match.driver_ride.id),'noreply@bervoets.covoiturage',[u'%s'%match.driver_ride.driver.email])
            return HttpResponseRedirect('/location/ride/matches/')
        else:
